...
import { Play, Pause, Video, Loader2, SkipBack, SkipForward } from 'lucide-react';
    ...
    return (
        <div className="bg-[#0D0D0D] border border-white/5 rounded-[2.5rem] px-12 py-7 flex flex-col gap-6 shrink-0 mb-1 shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)] relative overflow-hidden group">
            ...
            <div className="space-y-2">
                <div 
                    className="relative w-full h-2 bg-white/5 rounded-full cursor-pointer group/bar overflow-hidden"
                    onClick={(e) => {
                ...
                >
                    ...
                    <div 
                        className="absolute top-0 left-0 h-full bg-white transition-all duration-100 ease-out flex justify-end items-center"
                        style={{ width: `${progress}%` }}
                    >
                         <div className="w-4 h-4 bg-white rounded-full shadow-[0_0_20px_rgba(255,255,255,0.8)] translate-x-1/2 scale-0 group-hover/bar:scale-100 transition-transform duration-300" />
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between">
                {/* LEFT: EMPTY SPACE FOR BALANCE OR SMALL TRACK INFO */}
                <div className="flex-1" />

                {/* CENTER: TRANSPORT CONTROLS (CLEANED) */}
                ...
                {/* RIGHT: RENDER COMMAND */}
                <div className="flex-1 flex justify-end">
                    <button 
                        onClick={onStartExport} 
                        disabled={playlist.length === 0 || isRecording} 
                    ...
                    >
                        ...
                        <span className="relative flex items-center gap-3">
                            {isRecording ? <Loader2 className="animate-spin text-black" size={18} /> : <Video size={18} />} 
                            {isRecording ? 'RENDERING' : 'MAKER VIDEO'}
                        </span>
                    </button>
                </div>
            </div>
